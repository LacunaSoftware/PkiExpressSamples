...
function generatePrinterFriendlyVersion($pdfPath, $verificationCode)
{
    ...
    $signatureExplorer = new PadesSignatureExplorer(getPkiExpressConfig());
    // Set PKI default options. (see Util.php)
    getPkiDefaults($signatureExplorer);
    ...
    $pdfMarker = new PdfMarker(getPkiExpressConfig());
...
function _getSignerDescription($signer)
{
    ...
    if ($signer->signingTime != null) {
        $text .= ' em ' .  date("d/m/Y H:i:s", strtotime($signer->signingTime));
    }
    return $text;
}